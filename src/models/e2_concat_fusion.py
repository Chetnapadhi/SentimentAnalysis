...
class TweetEvalEmojiModel(nn.Module):
    ...
    def __init__(self, vocab_size: int, emoji_dim: int = 32, num_labels: int = 20):
        ...
        self.emoji_encoder = nn.Embedding(emoji_dim, vocab_size, padding_idx=None)
...
def pretrain_emoji_embeddings():
    ...
    model = TweetEvalEmojiModel(
        vocab_size=32,  # E2 emoji embedding dim
        emoji_dim=32,
        num_labels=20
    ).to(DEVICE)
...
class E2ConcatFusionModel(nn.Module):
    ...
    def __init__(
        ...
        emoji_vocab_size: int = 32,
        max_emojis: int = 8,
        pretrained_emoji_path: str = "models/emoji_embeddings/pretrained_emoji_32d.pt",
    ) -> None:
        super().__init__()
        ...
        self.emoji_encoder = nn.Embedding(32, 32, padding_idx=None)
        pretrained = torch.load("models/emoji_embeddings/pretrained_emoji_32d.pt", map_location="cpu")
